...
def upgrade_run_to_v3(run_dir: Path) -> None:
    ...
    for file_name, header in (
        ("candidate_setups.csv", AUDIT.CANDIDATE_HEADER_V3),
        ("trade_entries.csv", AUDIT.TRADE_ENTRY_HEADER),
        ("trade_outcomes.csv", AUDIT.TRADE_OUTCOME_HEADER),
        ("cycle_outcomes.csv", AUDIT.CYCLE_OUTCOME_HEADER),
        ("barrier_outcomes.csv", AUDIT.BARRIER_OUTCOME_HEADER),
        ...
        with path.open("r", encoding="utf-8", newline="") as handle:
            rows = list(csv.DictReader(handle))
        for row in rows:
            row["SchemaVersion"] = AUDIT.SCHEMA_VERSION_V3
            if file_name == "candidate_setups.csv":
                row["FeatureReadyV3"] = "true"
                for feature in AUDIT.CANDIDATE_V3_FEATURES:
                    row[feature] = (
                        "true" if feature == "TrappedBetweenLevels" else "1"
                    )
        write_csv(path, header, rows)
...
class DatasetAuditTests(unittest.TestCase):
    ...
    def test_v3_run_requires_and_retains_compact_features(self) -> None:
        raw_root = self.root / "raw"
        raw_root.mkdir()
        ...
        self.assertEqual(report["schema_version"], AUDIT.SCHEMA_VERSION_V3)
        self.assertEqual(report["totals"]["retained_candidates"], 1)
        with (self.root / "processed" / "merged_candidates.csv").open(
            "r", encoding="utf-8", newline=""
        ) as handle:
